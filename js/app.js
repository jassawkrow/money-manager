...
VIEWS.home = () => {
  ...
  const pct = target > 0 ? Math.min(100, Math.round((income / target) * 100)) : 0;
  ...
  return `
    ...
    <div class="card goal">
      ${ringSvg(pct, "var(--accent)")}
      <div class="goal-info">
        <div class="goal-title">Earn target</div>
        <div class="goal-amount">${fmtMoney(income)} <span class="muted" style="font-size:14px;font-weight:500;">of ${fmtMoney(target)}</span></div>
        <div class="goal-remaining">${pct >= 100 ? "🎉 Target reached!" : `${fmtMoney(Math.max(0,target-income))} to go`}</div>
      </div>
    </div>
...
function attachAfterRender(tab) {
  ...
  if (tab === "home") {
    document.getElementById("edit-target")?.addEventListener("click", () => editTargetSheet("monthlyTarget", "Monthly target"));
  ...
  if (tab === "settings") {
    ...
    view.querySelector('[data-action="edit-target"]')?.addEventListener("click", () => editTargetSheet("monthlyTarget", "Monthly target"));
